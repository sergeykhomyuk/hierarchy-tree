...
14. Nothing on the users path is requested **while authenticating** - not the whole database, not the users collection, not a single user record. The window this invariant bounds opens when the submission starts and closes when the lookup settles: from the first keystroke through to the moment a user id is in hand, the network log contains the secrets request and nothing else from this application's origin. This is the invariant that keeps the plaintext password table out of the client **during the credential exchange itself**, and it is asserted from the network log rather than argued.

    What happens **after** that window is invariant 97's business: once a session exists, the header fetches the users collection and finds the signed-in user's own record client-side. That request is outside this invariant's window by construction and decides nothing about access. It amends an earlier draft of this invariant, which claimed the header fetches a single record by id and that the collection is never fetched at all - both false against the real database (see ARCHITECTURE.md's decision log, 2026-08-15): `/secrets.json` resolves to each record's own internal `id` field, not to `/users.json`'s storage position, and Firebase serves no per-id path or indexed query for that id, so the only working fetch is the whole collection, filtered client-side. The collection does carry every user's plaintext password (`docs/reference.md`), and that same collection is already public and unauthenticated-fetchable directly from the database regardless of anything this app does - the trade-off this invariant now accepts is that a *signed-in* visitor's own browser also makes this request as ordinary traffic, once per page lifetime, which is a wider audit surface for that one visitor's own network log than the single-record fetch this invariant originally promised, not a new party gaining access to data that was not already public. Invariant 97d holds the phase-3 line: this phase's own fetch is still not the collection request phase 3's tree will make for a different purpose.
...
18. A response that is neither `null` nor a valid user id - an object, an array, an empty string, a boolean - is treated as a service problem (invariants 58-65), never as a no-match, because a malformed response is not evidence that the credential is wrong.
    ...
    The charset restriction is a security boundary, not fastidiousness. The id is written into a stored session record and compared against every record in the users collection to find the signed-in user's own one (invariant 97), so a hostile or corrupt value could otherwise carry a traversal or structural character into either place. The restriction is applied at the parse boundary, before the id is used for anything or written anywhere, so a hostile or corrupt value never becomes an identifier in the first place; a response carrying one is a malformed response and produces the service-problem state. An earlier draft of this paragraph also described percent-encoding the id into a request path - that path no longer exists (see invariant 97's own note and ARCHITECTURE.md's decision log, 2026-08-15), so the charset check is now the whole of this boundary rather than one half of a belt-and-braces pair.
...
97. The signed-in user's full name is obtained by fetching the users collection and matching the signed-in user's own record against it by id, client-side, **after** authentication has already succeeded. This is outside the window invariant 14 bounds - it happens only once a session exists and decides nothing about access - and invariant 14's own text says so from its side, so the two are one contract rather than two claims that have to be reconciled. The id compared against each record is still charset-restricted at the parse boundary (invariant 18a) even though it is no longer interpolated into a path, because it is still a comparison key a stored or resolved value could otherwise carry a hostile character into. The result is held for the lifetime of the tab's page, not persisted, so the session record stays exactly what invariant 72 says it is.
    ...
    97d. This request is distinct from anything phase 3 does, even though both now fetch the same collection. This phase parses the response only far enough to find one matching record and discards the rest without rendering, storing or otherwise carrying the other 32-odd records anywhere; phase 3's own fetch, for the tree, is a separate request this phase does not make, cache, or share a result with - the two are not merged here.
